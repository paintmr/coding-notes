...
可用的账号13800000003，13800000004，13400000001，13400000001，13500000001，13500000002，13500000003，13600000001，13600000002
...
@keyup.enter.**`native`** 表示监听组件的原生事件，比如 keyup就是于input的原生事件，这里写native表示keyup是一个原生事件
 ...
 @keyup.enter.native="handleLogin"写在了el-input上面（如下代码所示）。而el-input盒子里面才是真正的原生HTML input。native表示监听的是el-input盒子里面的input。

而在element-ui中，一些组件（比如按钮）开放了click事件，就不用写native。
...
```js
 // 代理跨域的配置
    proxy: {
      ...
      '/api': {
        target: 'http://ihrm-java.itheima.net/', // 跨域请求的地址。要代理的服务器地址，这里不用写api
...
> 我们需要知道,**`钥匙`**不能每次都通过登录获取,我们可以将token放置到本地的缓存中，以持久化保存，避免刷新页面就没有了。
...
service.interceptors.response.use(response => {
  ...
  return Promise.reject(error) // 把错误对象error传递给Promise.reject()。返回执行错误 让当前的执行链跳出成功 直接进入 catch
})
```
