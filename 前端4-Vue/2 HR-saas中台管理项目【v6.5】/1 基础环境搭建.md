...
同时，请注释掉**`vue.config.js`**中的  **before: require('./mock/mock-server.js')**  // 这行代码指 项目启动之前，会去引入mock-server.js文件
...
> 为了方便应用scss，我们可以在vscode中安装一个名为**`easy sass`** 的插件，但是我们只在该项目中工作区中应用该插件，因为在项目中，不需要该插件的辅助 （@vue/cli，webpack loader会自动把scss代码转换为css代码）
