...
```js
 async  success({ header, results }) {
    ...
    formatDate(numb, format) {
      ...
      const year = time.getFullYear() + ''
      const month = time.getMonth() + 1 + ''
     // const date = time.getDate() - 1 + '' 这行代码错了
...
cos-js-sdk-v5是腾讯云的包。如果不用腾讯云，可以不装这个包。
...
```js
    // on-change是文件状态改变时的钩子，添加文件、上传成功和上传失败时都会被调用。所以这里不能用fileList.push()，否则push()会被多次调用。
	// file是被选中的文件，fileList是最新的文件列表
	// 这里也不能用this.fileList = fileList
    // 如果在下面的changeFile中console.log(fileList.length),会发现打印了两次：第一次是1，第二次是0。因为选择好文件后，会调用一次changeFile,此时打印fielList.length，结果是1。上传成功/失败后，会再调用一次changeFile()。我们在开发时，因为预计后续要自己设置上传请求，在template中的<el-upload中设置了action="#"。按照官方例子，这里应该是action="接收文件的URL地址"。所以此时上传肯定是失败的，會再次進入changeFile()，同时系统会删掉刚刚加入fileList中那个文件file，所以此时打印fielList.length，结果是0.
    // 这里先写成这样，等上传成功了，再来继续写相关代码。
...
注册可以在main.js，也可以在src/components/index.js
